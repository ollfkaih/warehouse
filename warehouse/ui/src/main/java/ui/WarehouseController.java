...
public class WarehouseController {
  ...
  private List<Item> getItems() {
    ...
    //TODO: When adding sorting options, call getAllItemsSorted with params
    return items
        .stream()
  ...
  private void updateInventory() {
    ...
    for (int i = 0; i < items.size(); i++) {
      ...
      itemPaneList.get(i).getChildren()
          .addAll(
              textName,
              textStatus,
              textAmountText,
  ...
  private void setButtonPosition(Button button, int layoutX, int layoutY) {
    button.setLayoutX(layoutX);
    button.setLayoutY(layoutY);
  }

  private void setTextProperties(Text textField, int layoutX, int layoutY, Font font) {
    setTextProperties(textField, layoutX, layoutY);
    textField.setFont(font);
  }

  private void setTextProperties(Text textField, int layoutX, int layoutY) {
